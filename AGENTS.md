...
| **A test fixture whose value coincides with the implementation's default proves nothing.** | This shipped **three separate times** during the build and was caught only in review, never by the suite: an arbitration test whose fixtures never let recency and id order disagree (a transition-blind implementation passed it); a `trackid` test whose fixture contained no character to sanitise (deleting the sanitiser passed it); an `artUrl` test using the same port as the fallback (ignoring the state value entirely passed it). When writing a fixture, pick a value the implementation could not produce by accident. |
